...
namespace LCMS\Core\Pages{
	...
	class PageLog{
		...
		const EDIT=3;
		const DELETE=4;
		public static function action($path, $user, $type, $ok=true){
			switch($type){
				case (self::ADD):
			...
			$type="\r\n$path|$user|$type|$ok";
			file_put_contents("page_log.db", trim(file_get_contents("page_log.db").$type));
		}
